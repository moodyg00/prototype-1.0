...
export function PhotographyPanelView({
  context: _context,
}: {
  toolId: ToolId;
  ...
  async function quickGenerate() {
    ...
    try {
      ...
      toast.success(data.stub ? 'Stub saved — open studio for models' : 'Generated');
    } catch (e) {
  ...
  return (
    <div className="flex h-full flex-col gap-2 p-3">
      <p className="text-[10px] uppercase tracking-wider text-zinc-600">Photography</p>
      <textarea
      ...
      <button
        ...
        className="inline-flex items-center justify-center gap-1 rounded bg-violet-600/90 py-2 text-[11px] text-white"
      >
        <Sparkles size={12} />
        Generate
      </button>
      <button
        type="button"
        onClick={() => dispatchAgentNavigate({ toolId: 'photography' })}
