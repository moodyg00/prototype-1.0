...
export function DialogViewport({
  ...
  return (
    <DialogPrimitive.Viewport
      className={cn(
        contained ? "absolute inset-0" : "fixed inset-0",
        "z-50 grid grid-rows-[1fr_auto_3fr] justify-items-center p-4",
        className,
      )}
...
export function DialogPopup({
  ...
  /** When set, portal into this element and use absolute (not fixed) overlay positioning. */
  portalContainer?: HTMLElement | null;
}): React.ReactElement {
  const contained = Boolean(portalContainer);

  return (
    <DialogPortal {...portalProps} container={portalContainer ?? portalProps?.container}>
      <DialogBackdrop contained={contained} />
      <DialogViewport
        contained={contained}
        className={cn(
          bottomStickOnMobile &&
